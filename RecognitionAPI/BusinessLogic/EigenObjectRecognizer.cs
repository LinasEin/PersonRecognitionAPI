...
namespace WhosThat
{
   ...
   public class EigenObjectRecognizer
   {
      ...
      {
         Debug.Assert(condition: images.Length == labels.Length, message: "The number of images should equals the number of labels");
         Debug.Assert(condition: eigenDistanceThreshold >= 0.0, message: "Eigen-distance threshold should always >= 0.0");

         CalcEigenObjects(trainingImages: images, termCrit: ref termCrit, eigenImages: out _eigenImages, avg: out _avgImage);
      ...
      public static float[] EigenDecomposite(Image<Gray, Byte> src, Image<Gray, Single>[] eigenImages, Image<Gray, Single> avg)
      {
         return CvInvoke.cvEigenDecomposite(
             obj: src.Ptr,
             eigInput: Array.ConvertAll<Image<Gray, Single>, IntPtr>(eigenImages, delegate(Image<Gray, Single> img) { return img.Ptr; }),
             avg: avg.Ptr);
      }
      #endregion
